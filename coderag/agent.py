...
try:
    ...
    logger.info(
        "Gemini client initialized successfully " f"with model: {GEMINI_CHAT_MODEL}"
    )
...
def call_gemini_with_retry(prompt: str) -> str:
    """
    Safely call Gemini model with automatic retries.
    """
...
def run_agent(user_query: str) -> str:
    ...
    logger.info(
        f"Tool '{tool_name}' output length: "
        f"{len(tool_output) if tool_output else 0}"
    )
